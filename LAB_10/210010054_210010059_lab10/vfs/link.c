...
// To print when a file is deleted asiignemnt 9
...
/* Truncate a regular file or a pipe */
  ...
  if (!S_ISFIFO(vp->v_mode) && !(S_ISREG(vp->v_mode) || (vp->v_mode & S_IFMT) == 0110000)) return(EINVAL);  /* Assignment 10 */
