...
static void removeInodeEntry(struct inode *rip);	/* Assignment 10 */
...
int fs_readwrite(void)
{
  ...
  regular = (mode_word == I_REGULAR || mode_word == I_NAMED_PIPE || mode_word==I_IMMEDIATE);  /* Assignment 10 */
...
/* Assignment 10 */
  if(mode_word == I_IMMEDIATE) {  // Check if the file is an immediate file
    if(rw_flag == WRITING) {
      if(position + nrbytes > 32) { // // Check if the total bytes to be written exceeds the immediate file size limit
        ...
        for(int i = 0; i < f_size; ++i) {   // Copy data from inode zones to buffer
          if(i%4 == 0)
            current_bytes = (char*)rip->i_zone + i;
          buffer[i] = current_bytes[i%4];
        }
        removeInodeEntry(rip);  // Remove inode entry and convert inode to regular file
        rip->i_mode = (I_REGULAR | (rip->i_mode & ALL_MODES));
        mode_word = rip->i_mode & I_TYPE;
        if((bp = new_block(rip, (off_t) ex64lo(position))) == NULL) // Allocate a new block for the file and write data to it
          return(err_code);
        for(i = 0; i < f_size; ++i) {
        ...
        immediate = 1; // If writing operation but not exceeding immediate file size limit, set immediate flag
      }
    }
  ...
  if(immediate == 1) {  // If immediate flag is set, perform immediate file operations
    if(rw_flag == READING) {
      printf("Minix 210010054_210010059: Reading from Immediate File\n");
      // Copy data from inode zones to user space
      r = sys_safecopyto(VFS_PROC_NR, gid, (vir_bytes)cum_io,(vir_bytes) rip->i_zone,(size_t) f_size);
      // Copy data from inode zones to buffer for printing
      int position = 0;
      char* current_bytes;
      char buffer[40];	
      ...
      printf("Minix 210010054_210010059: Content of Immediate File:\n");
      for(i = 0; i < f_size; ++i) {
        printf("%c", buffer[i]);
      }
      printf("Minix 210010054_210010059: E0F Immediate File\n");
      
      if(r == OK) { // If read operation successful, update counters
        nrbytes=0;
        cum_io += f_size;
        position += f_size;
      }
      ...
      printf("Minix 210010054_210010059: Writing to Immediate File\n");
      vir_bytes zone;
